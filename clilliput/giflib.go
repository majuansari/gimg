...
func (d *gifDecoder) Header() (*ImageHeader, error) {
	return &ImageHeader{
		width:       float64(C.giflib_decoder_get_width(d.decoder)),
		height:      float64(C.giflib_decoder_get_height(d.decoder)),
...
func (d *gifDecoder) FrameHeader() (*ImageHeader, error) {
	return &ImageHeader{
		width:       float64(C.giflib_decoder_get_frame_width(d.decoder)),
		height:      float64(C.giflib_decoder_get_frame_height(d.decoder)),
		pixelType:   PixelType(C.CV_8UC4),
		orientation: OrientationTopLeft,
		numFrames:   1,
	}, nil
...
func (d *gifDecoder) DecodeTo(f *Framebuffer) error {
	...
	if frameHeader.Width() > float64(maxDim) || frameHeader.Height() > float64(maxDim) {
		return ErrInvalidImage
	}
